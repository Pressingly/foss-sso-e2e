...
def test_update_merges_fields(state_file):
    st = StateTracker(state_file)
    st.update("issue-1", status="test_written", test_path="/tmp/bug_abcd.spec.ts")
    entry = st.get("issue-1")
    assert entry["status"] == "test_written"
    assert entry["test_path"] == "/tmp/bug_abcd.spec.ts"
...
def test_mark_failed_and_reset(state_file):
    ...
    assert st.get("issue-1")["status"] == "test_written"
...
def test_state_survives_reload(state_file):
    st = StateTracker(state_file)
    st.update("issue-1", status="test_written", test_path="/x/bug_abc.spec.ts")

    st2 = StateTracker(state_file)
    entry = st2.get("issue-1")
    assert entry["status"] == "test_written"
    assert entry["test_path"] == "/x/bug_abc.spec.ts"
