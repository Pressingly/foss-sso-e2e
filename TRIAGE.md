...
| `spec-coverage` check failed with `FATAL: could not enumerate requirements` | Vendor is missing — restore `vendor/openspec/specs/` or set `SPEC_DIR` explicitly | Inspect the vendor directory; restore from git history or a known-good checkout. |
