...
| **Spec source — vendored** | The SSO openspec lives at `vendor/openspec/specs/` (snapshot of `awais786/sso-rules-moneta`). The audit defaults to it — no network or token at CI time. Refresh from a local upstream checkout: `SSO_RULES_SRC=/path/to/sso-rules-moneta bash scripts/refresh-openspec.sh`, review the diff, commit. `vendor/openspec/skills/` carries the per-app admin contracts (one `SKILL.md` per app) + the devstack `app-rules/RULES.md` — vendored as reference; the audit doesn't parse them yet (they don't use the `### Requirement:` format). |
