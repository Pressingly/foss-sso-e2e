...
| **Outline** | Rate-limits chunk loads under burst → returns HTTP 429 on `/settings/*` chunk requests when the suite hits multiple pages in quick succession. Not a test bug. `outline-admin.spec.ts` handles it inline: a network listener counts 429 responses per attempt, retries use exponential backoff (15s × attempt) when a 429 is seen, 1.5s otherwise. Per-test timeout is bumped to 180s to fit the worst-case backoff budget. Same pattern can be ported to other Outline-heavy tests if 429s start appearing there. |
