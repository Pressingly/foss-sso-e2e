...
| **Plane (PM)** | God-mode uses local creds, NOT SSO (`pm-godmode.spec.ts`). The shared SSO workspace is `fossarbisoft` — both `FOSS_USER` and `NORMAL_USER` are auto-joined as **Member** there (the auto-join-role contract `pm-admin.spec.ts` pins); only `system-bot@foss.arbisoft.com` has Admin role. A separate `aa` workspace exists where `FOSS_USER` IS Owner, but the shared admin tests use `fossarbisoft`. Slug overridable via `PLANE_ADMIN_WORKSPACE_SLUG`. |
...
| **Identities** | `FOSS_USER` is "User A" — Outline `role=admin`, Penpot/SurfSense Owner of shared team, Twenty `canAccessFullAdminPanel=true`, Plane Owner of `aa` / Member of `fossarbisoft`. `NORMAL_USER` is "User B" — non-admin baseline everywhere; non-admin tests self-skip when unset. `PLANE_ADMIN_USER`/`PLANE_ADMIN_PASS` is the local-creds god-mode admin (separate from SSO). |
