...
| **Plane (PM)** | God-mode uses local creds, NOT SSO (`pm-godmode.spec.ts`). The shared SSO workspace is `fossarbisoft` (the "SMB" shared workspace per `foss-server-bundle/scripts/provision-admin/`). After SMB provisioning, **`FOSS_USER` is workspace Owner** of `fossarbisoft` (was previously auto-joined as Member — the provisioning promoted it). `NORMAL_USER` still auto-joins as Member. `system-bot@foss.arbisoft.com` is the service-account Admin used by the provisioning scripts. Slug overridable via `PLANE_ADMIN_WORKSPACE_SLUG`; workspace UUID via `PLANE_WORKSPACE_ID`. |
...
| **Identities** | `FOSS_USER` is "User A" — after SMB-workspace provisioning (`foss-server-bundle/scripts/provision-admin/`) the roles are: Outline **team admin on the SMB team** (`OUTLINE_TEAM_ID`), Penpot Owner of the SMB team (`PENPOT_TEAM_ID`), SurfSense Owner of the SMB search space (`SURFSENSE_SEARCH_SPACE_ID`), Plane Owner of `fossarbisoft` (`PLANE_WORKSPACE_SLUG` / `PLANE_WORKSPACE_ID`), Twenty workspace Admin (NOT instance admin — that needs separate `bootstrap-sso-admin` + DB flag for `canAccessFullAdminPanel`). `NORMAL_USER` is "User B" — non-admin baseline everywhere; auto-joins as Member; non-admin tests self-skip when unset. `system-bot@foss.arbisoft.com` is the service-account used by SMB provisioning scripts — it holds higher-level workspace/instance admin roles in some apps but isn't loginable from Playwright (no Cognito password). `PLANE_ADMIN_USER`/`PLANE_ADMIN_PASS` is the local-creds god-mode admin (separate from SSO). |
