...
| `tests/apps/pm-workspace-isolation.spec.ts` | NORMAL_USER's UI navigation AND API requests to FOSS_USER's private workspace (`FOSS_USER_PRIVATE_WORKSPACE_SLUG`, default `aa`) are both refused — UI shows "Workspace not found"; API returns 4xx. Pre-condition assertions confirm NORMAL_USER's own workspace IS reachable both ways. | Workspace-membership authorization is a Plane-internal contract: a valid SSO session must not grant cross-workspace data access. Not in the openspec (which terminates at the SSO chain); here as defense-in-depth + a regression guard if Plane's view-level check is ever loosened. |
...
| `tests/security/headers.spec.ts` (hardening rules beyond the openspec baseline) | **Portal only:** CSP with `default-src` and no `'unsafe-inline'` in `script-src`; COOP=`same-origin`; CORP set. **All hosts:** Server header must not leak upstream version (`nginx/<n>`, `apache/<n>`). | None of these are in the openspec but they're standard defense-in-depth controls. CSP/COOP/CORP are intentionally portal-only — the bundle owns the portal HTML so it can pick a sensible policy, but for the 5 apps each ships their own (Penpot/Plane/SurfSense/Twenty deliberately don't; Outline does). Bundle-imposed CSP would break the apps' SPA needs. |
